...
-- tabla intermedia para usuarios y condiciones clinicas
...
-- ==========================================
-- INSERCION DEL USUARIO ADMINISTRADOR DEL SISTEMA (CARLA NUESTRA CLIENTE)
-- ==========================================
...
-- Insertar categorias oficiales precargadas
...
-- insertar ordenes de pedido asociadas a los clientes test
...
-- asociar productos a las ordenes con cantidad
...
BEGIN
    ...
    IF UPDATING THEN
        ...
        -- si la cantidad subio, restar la diferencia al stock
        ...
        -- si la cantidad bajo, devolver la diferencia al stock
        IF :NEW.quantity < v_old_quantity THEN
            UPDATE product
    ...
    -- manejar DELETE (Eliminacion de producto en la orden, devolver stock)
    IF DELETING THEN
        UPDATE product
...
BEGIN
    -- recorrer todos los productos de la orden cancelada y restaurar el stock
...
-- PROCEDIMIENTO ALMACENADO PARA DESCUENTO MASIVO (funcion y procedimiento)
...
BEGIN
    ...
    -- devuelve el nuevo descuento aplicado
    RETURN p_discount;
END;
/

-- procedimiento almacenado que ejecuta funcion para aplicar descuento por categoria
...
BEGIN
    -- recorrer todos los productos activos de la categoria seleccionada
    FOR prod IN (
        SELECT product_id 
        FROM product
        ...
        -- aplicar el descuento usando la funcion y almacenarlo en una variable
        v_new_discount := apply_discount_to_product(prod.product_id, p_discount);
        
        -- actualizar manualmente el producto (por si la funcion no lo hace bien)
        UPDATE product
